...
![](/images/1692669066608-256.png)
![](/images/1692669134994-184.png)
...
- domain：域，表达这个cookie是属于哪个网站的，比如`example.com`，表示这个cookie是属于`example.com`这个网站的
...
- cookie中的域和这次请求的域是匹配的
    - 比如cookie中的域是`example.com`，则可以匹配的请求域是`example.com`、`www.example.com`、`blogs.example.com`等等
    - 比如cookie中的域是`www.example.com`，则只能匹配`www.example.com`这样的请求域
...
- **domain**：设置cookie的域。如果不设置，浏览器会自动将其设置为当前的请求域，比如，浏览器请求的地址是`http://www.example.com`，服务器响应了一个`set-cookie: a=1`，浏览器会将该cookie的domain设置为请求的域`www.example.com`
    - 这里值得注意的是，如果服务器响应了一个无效的域，浏览器是不认的
    - 什么是无效的域？就是响应的域连根域都不一样。比如，浏览器请求的域是`example.com`，服务器响应的cookie是`set-cookie: a=1; domain=baidu.com`，这样的域浏览器是不认的。
...
下面来一个例子，客户端通过`post`请求服务器`http://example.com/login`，并在消息体中给予了账号和密码，服务器验证登录成功后，在响应头中加入了以下内容：
...
```yaml
cookie: token=; domain=example.com; path=/; max-age=-1
```
